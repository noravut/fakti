...
ขอบเคส, จุดเสี่ยง, **ช่องโหว่ของ REQ** 4 แบบ, คำถามที่ติด) → ขั้น 2 ลงมือทีละ REQ commit ใส่ `REQ-n`
→ ขั้น 3 ตารางตรวจเอง REQ × เกณฑ์ × หลักฐาน × ไฟล์ × สถานะ + ช่องโหว่ที่ยังเปิดอยู่

ด่านหยุดในขั้น 1: มีคำถามที่ติด หรือช่องโหว่แบบ "ขัดกันเอง"/"ขาดกฎ" → รอคำตอบ
ส่วน "ขาดคู่"/"ชนของเดิม" เสนอไว้แล้วทำต่อภายใต้ข้อสมมติ — กันไม่ให้ถามทุกครั้งจนช้า

ถามได้ไม่เกิน 3 ข้อต่อรอบ เกินนั้นเรียงตามผลกระทบแล้วยกมา 3 ข้อ ที่เหลือตั้งข้อสมมติ
ติดป้าย "ยังไม่ยืนยัน" แล้วยกกลับมาถามรอบถัดไป — คนตอบเกิน 3 ข้อพร้อมกันจะตอบผ่าน ๆ

agent เขียน spec ลง `.pat-spec.md` และรายงาน QA ลง `.pat-qa-report.md` ด้วย ไม่ใช่พิมพ์ออกจออย่างเดียว
เพราะก็อปข้อความยาวจากเทอร์มินัลแล้วตกหล่น · ทั้งสองไฟล์ถูก `excludeReportFiles` กันออกจาก git ตอนเปิด session
